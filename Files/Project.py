...
# Use local file paths instead of Google Drive paths
...
# Remove leading and trailing whitespace from each value in the 'course_code' column
ip_1['course_code'] = ip_1['course_code'].str.strip()

# Display the first 10 rows of the DataFrame ip_2
ip_2.head(10)

# Display the first 10 rows of the DataFrame ip_3
ip_3.head(10)

# Display the first 10 rows of the DataFrame ip_4
ip_4.head(10)

# Get the counts of unique values in the 'course_code' column
ip_1.course_code.value_counts()

ip_3   #Print all info

# Define the extract_floor function
...
buffer = int(input("write the amount of buffer you want in room in percentage (e.g., 5):  "))/100
...
# seating is available in Excel file and CSV file
expanded_op_1_sorted.to_excel("room_assignment.xlsx", index=False)

expanded_op_1_sorted.to_csv("room_assignment.csv", index=False)
